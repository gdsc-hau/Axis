...
sequenceDiagram
    ...
    Hub->>Db: Validate member exists & member_status = ACTIVE
    Note over Hub,Db: Skip if already activated or not active
    ...
    Hub->>Db: RPC verifies email, then links auth_id and records activated_at
...
sequenceDiagram
    ...
    Db-->>API: Return member (member_status = ACTIVE)
...
6. **Membership Access**: `members.member_status` is the lifecycle source of truth. Only `ACTIVE` members resolve through member-owned RLS helpers or active-member/application guards. `is_accepted` is a derived compatibility field only.

...
import { BookOpen, Users, Trophy, Briefcase, Star, Medal, Award, Handshake, CreditCard } from 'lucide-react';
...
const colorStyles: Record<string, string> = {
  ...
  green: 'bg-green-500/10 text-green-400 border-green-500/20 group-hover:border-green-500/50 group-hover:shadow-[0_0_15px_rgba(52,168,83,0.3)]',
  yellow: 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20 group-hover:border-yellow-500/50 group-hover:shadow-[0_0_15px_rgba(251,188,5,0.3)]',
  ...
  purple: 'bg-purple-500/10 text-purple-400 border-purple-500/20 group-hover:border-purple-500/50 group-hover:shadow-[0_0_15px_rgba(155,81,224,0.3)]',
  orange: 'bg-orange-500/10 text-orange-400 border-orange-500/20 group-hover:border-orange-500/50 group-hover:shadow-[0_0_15px_rgba(249,115,22,0.3)]',
};
  ...
  return (
    <div className="w-full max-w-5xl mx-auto my-16 pt-8 border-t border-white/10 relative">
      <div className="absolute top-0 right-0 -translate-y-1/2 w-16 h-16 sm:w-24 sm:h-24">
        <Image src="/assets/images/gyro/surprise_gyro_icon.png" alt="Gyro Mascot" width={96} height={96} className="w-full h-full object-contain animate-bounce" />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {benefits.map((benefit, i) => {
          ...
          return (
            ...
            >
              <div className={`p-4 rounded-full mb-4 border transition-all duration-300 ${colorStyles[benefit.color]}`}>
                <Icon className="w-8 h-8" />
              </div>
              <h4 className="text-white font-semibold text-center">{benefit.title}</h4>
            </motion.div>
          );
        })}
      </div>

...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ip) return NextResponse.json({ error: 'Unable to determine client IP.' }, { status: 400 });
    ...
    if (!claims) return NextResponse.json({ error: 'Invalid or expired verification token.' }, { status: 401 });
    ...
    const { data: member, error } = await supabase
      ...
      .single();
    if (error || !member?.is_accepted) return NextResponse.json({ error: 'Member is not active.' }, { status: 404 });

    return NextResponse.json(PublicMemberProfileSchema.parse({
      gdgId: member.gdg_id,

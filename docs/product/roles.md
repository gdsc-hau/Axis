...
3. **UI Level:**
   Frontend components conditionally render based on the user's role. For example, the "Create Event" button is entirely hidden from `MEMBER` users. *Note: UI hiding is for UX, not security. RLS provides the actual security.*

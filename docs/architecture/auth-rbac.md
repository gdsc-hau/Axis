...
We _never_ query `auth.users` directly in our application code. Instead, we use a custom public table called `public.members`.

### The `auth_id` Bridge

Unlike a typical Supabase project where `auth.users.id` and `public.members.id` share the same UUID via a trigger, Axis uses a **pre-populated members table** as the source of truth. Members are added by admins _before_ they have a Supabase auth account.
...
Because Next.js Server Components and Server Actions pass the user's JWT to Supabase securely, Supabase knows exactly _who_ is making the request.
